    ...
    <form method="POST" action="{{ route('admin.menu.update', $menu->id) }}" enctype="multipart/form-data">
      ...
      <div class="grid sm:grid-cols-12 gap-2 sm:gap-4 py-8 first:pt-0 last:pb-0 border-t first:border-transparent border-gray-200 first:border-transparent">
        ...
        <div class="sm:col-span-9">
          <input id="nama" type="text" name="nama" value="{{ old('nama', $menu->nama) }}" class="py-1.5 sm:py-2 px-3 pe-11 block w-full border border-gray-300 shadow-sm rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none {{ $errors->has('nama') ? 'border-red-500' : '' }}" required>
          @if($errors->has('nama'))
        ...
        <div class="sm:col-span-9">
          <select id="category_id" name="category_id" class="py-1.5 sm:py-2 px-3 pe-11 block w-full border border-gray-300 shadow-sm rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none {{ $errors->has('category_id') ? 'border-red-500' : '' }}" required>
            ...
            @foreach($categories as $category)
              <option value="{{ $category->id }}" {{ old('category_id', $menu->category_id) == $category->id ? 'selected' : '' }}>{{ $category->nama }}</option>
            @endforeach
          </select>
        ...
        <div class="sm:col-span-9">
          <input id="harga" type="text" name="harga" value="{{ old('harga', $menu->harga ?? '') }}" class="py-1.5 sm:py-2 px-3 pe-11 block w-full border border-gray-300 shadow-sm rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none {{ $errors->has('harga') ? 'border-red-500' : '' }}" required>
          @if($errors->has('harga'))
      ...
      <div class="grid sm:grid-cols-12 gap-2 sm:gap-4 py-8 first:pt-0 last:pb-0 border-t first:border-transparent border-gray-200 first:border-transparent">
        ...
        <div class="sm:col-span-9">
          <div id="photoPreview" class="mt-6">
            @if($menu->foto)
              <img id="previewImage" src="{{ asset($menu->foto) }}" alt="Preview" class="max-w-sm rounded-lg shadow-lg">
            @else
              <div class="text-gray-400 text-sm">Belum ada foto</div>
            @endif
          </div>
        </div>
        <!-- End Col -->
